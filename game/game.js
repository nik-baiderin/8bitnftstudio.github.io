document.addEventListener("DOMContentLoaded", function () {
  ...
  const energyScore = document.querySelector(".energy_score");
  const energyBarWidth = energyBar.clientWidth;
  const energyBarValue = document.querySelector(".energybar_value");
  ...
  const closeMenuPopupButton = document.getElementById(
  ...
  const setProggressBarValue = (value) => {
    progressBar.value = value;
    energyScore.textContent = value;
    energyBarValue.style.width = `${(energyBarWidth / 200) * value}px`;
  };
  ...
  function getQuestions() {
    fetch("https://gptkids.online/clicker-backend/api/question", {
      ...
      .then((qData) => {
        ...
        document.querySelector(".quiz_answers").innerHTML = "";
        answers.map((answer) =>
          document
            .querySelector(".quiz_answers")
            .insertAdjacentHTML(
              "beforeend",
              `<button class="btn btn-primary quiz_answer_btn w-100" data-is-correct="${answer.isCorrect}">${answer.text}</button>`
            )
        );
        ...
        document
          ...
          .addEventListener("click", (e) => {
            if (!e.target.classList.contains("quiz_answer_btn")) return;
  ...
  function handleAnswerClick(answer) {
    if (answer.isCorrect) {
      ...
      document.querySelector(".quiz_result").classList.add("_active");
    } else {
      console.log("Incorrect answer.");
    }

    [...document.querySelectorAll(".quiz_answer_btn")].map((el) => {
      const isCorrect = JSON.parse(el.dataset["isCorrect"]);
      if (isCorrect) {
  ...
  function loadCount() {
    ...
    fetch(`https://gptkids.online/clicker-backend/api/user/${telegramID}`, {
      ...
      .then((data) => {
        ...
        setProggressBarValue(loadenergy);

        if (!counter || counter <= 0) {
          document.querySelector("#onboardingLink").click();
        }
  ...
  function checkEnergy() {
    ...
    if (serverDate == currentDate) {
      ...
      setProggressBarValue(
        progressBar.value + currentRestore * sixSecondIntervals
      );

      // progressBar.value += finalenergy;
      setProggressBarValue(progressBar.value + finalenergy);
      if (progressBar.value > progressBar.max) {
        // progressBar.value = progressBar.max;
        setProggressBarValue(progressBar.max);
      }
  ...
  const closeUpgradePopupButton = document.getElementById(
  ...
  const closeSkinPopupButton = document.getElementById(
    "close-skin-popup-button"
  );

  // document
  //   .getElementById("skin-button-1")
  //   .addEventListener("click", function () {
  //     handlePurchase(1, "images/click1.png");
  //   });

  // document
  //   .getElementById("skin-button-2")
  //   .addEventListener("click", function () {
  //     handlePurchase(2, "images/click2.png");
  //   });

  // document
  //   .getElementById("skin-button-3")
  //   .addEventListener("click", function () {
  //     handlePurchase(3, "images/click3.png");
  //   });

  // document
  //   .getElementById("skin-button-4")
  //   .addEventListener("click", function () {
  ...
  openPopupButton.addEventListener("click", function () {
    if (!popupCooldown) {
      togglePopup(true);
      popupCooldown = true;
      openPopupButton.classList.remove("_active");
  ...
  clickCircle.addEventListener("click", function () {
    ...
    if (progressBar.value > 0) {
      ...
      setProggressBarValue(progressBar.value - energyDrainPerClick);
      // sliderValueElement.textContent = progressBar.value;
    }
  ...
  setInterval(function () {
    if (progressBar.value < progressBar.max) {
      ...
      setProggressBarValue(progressBar.value + currentRestore);
      // sliderValueElement.textContent = progressBar.value;
    }
  }, 6000);
  ...
  //     uplevel++;
  //     upupgradePrice =
  //       (1 + (uplevel - 1) * (uplevel - 1)) * 10 * (uplevel * 2 + 1);
  //     updateUpDisplay();
  //     updateUpPrice();
  //     counterElement.textContent = counter;
  //     if (uplevel === 5) {
  ...
  //     restorelevel++;
  //     restoreupgradePrice =
  //       (1 + (restorelevel - 1) * (restorelevel - 1)) *
  //       10 *
  //       (restorelevel * 2 + 1);
  //     updateRestoreDisplay();
  //     updateRestorePrice();
  ...
  function togglePopup(show) {
    if (show) {
      document.querySelector(".quiz").classList.add("_active");
    } else {
      document.querySelector(".quiz").classList.remove("_active");
    }
    // if (show) {
  ...
  function updateRestorePrice() {
    if (!RestorePriceDisplay) return;
    RestorePriceDisplay.textContent =
      "Стоимость улучшения: " + restoreupgradePrice;
    if (restorelevel == 5) {
  ...
  function toggleButtonVisibility() {
    if (!popupCooldown) {
      if (openPopupButton.classList.contains("_active")) {
        openPopupButton.classList.remove("_active");
        setTimeout(toggleButtonVisibility, 20000);
      } else {
        openPopupButton.classList.add("_active");
